...
const useSpotifyHandler = (redirect?: "off" | "alert") => {
  const { credentials, redirectWithImplicitGrantFlow, setAccessToken } =
    useSpotify();
  ...
  const handleReceiver = useCallback(() => {
    ...
    if (error) throw new Error((error as string) ?? "Spotify Error");
      ...
      const redirectUrl = state?.includes(window.location.hostname)
        ? (state as string)
        : "";
  ...
  const doRedirect = useCallback(() => {
    if (
      ...
      (redirect === "alert" &&
        ...
        confirm(
          "Your Spotify Connection has expired, do you want to refresh it now?"
        ))
    )
  ...
  const handleRedirector = useCallback(() => {
    if (redirect !== "off" && accessTokenStorage && accessTokenExpiry) {
      const timeUntilExpiry =
        new Date(accessTokenExpiry).getTime() - getMinutes(5) - Date.now();
